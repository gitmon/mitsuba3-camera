...
class RadialWarpCamera final : public ProjectiveCamera<Float, Spectrum> {
public:
    ...
    RadialWarpCamera(const Properties &props) : Base(props) {
        ScalarVector2i size = m_film->size();
        m_x_fov = (ScalarFloat) parse_fov(props, size.x() / (double) size.y());
        m_k_warp = props.get<ScalarFloat>("k_warp", 1.0f);
    ...
    void traverse(TraversalCallback *callback) override {
        ...
        // TODO: which are the correct flags?
        callback->put_parameter("k_warp",                   m_k_warp,                     ParamFlags::Differentiable | ParamFlags::Discontinuous);
    ...
    void update_camera_transforms() {
        ...
        // NOTE: `sample` space refers to screen/raster space, but the 
        // coords are normalized to [0,1] and cropped to match the film/
        // screen size.
        ...
        // NOTE: recall that the coords are normalized to the [0,1] range, so
        // the pixels are of size 1 / res_x and 1 / res_y respectively
        ...
        // TODO
        // seems to refer to the top-left and bottom-right extremes of the
        // raster/screen area
        ...
        // TODO: not sure what this does. i think it allows the sample_ray()
        // method to make use of an additional random (2D) sample 
        // (3rd sample overall; 1st is 1D for spectral sampling, 2nd is the 
        // usual 2D sample for, presumably, hemispherical sampling)
    ...
    std::pair<Ray3f, Spectrum> sample_ray(Float time, Float wavelength_sample,
        ...
        // NOTE: `scaled_principal_point_offset` is the location of the pin-hole
        ...
        // convert ray from camera space to world space; translate the origin, and 
        // apply the rotation coord transform on the ray direction
    ...
    sample_ray_differential(Float time, Float wavelength_sample, const Point2f &position_sample,
        ...
        // the ray and its differentials come from the same point
        ray.o_x = ray.o_y = ray.o;

        // adjust the directions of the differential rays s.t. they pass through
        // the (i+1)-th (dx) and (j+1)-th (dy) pixels respectively. note that 
        // `m_dx` and `m_dy`, as computed in the constructor, are already in camera
        // space
    ...
    sample_direction(const Interaction3f &it, const Point2f & /*sample*/,
        ...
        // NOTE: this is *camera* space, NOT clip space
        ...
        // first, check if the point lies outside the near/far planes
        ...
        // then, check if it's inside the crop area (of course, we first need
        //  to convert the point into screen/raster space)
        ...
        // UV coordinates are in pixel units (e.g. 0 - 1080)
        ds.uv *= m_resolution;
        ...
        // the `other` point is the pinhole location (TODO: not the screen?)
        // transform `p` to world coordinates
        ds.p    = trafo.transform_affine(Point3f(0.0f));
        ds.d    = (ds.p - it.p) * inv_dist;     // could also coord-transform local_d
        ds.dist = dist;
        // world-space normal of the sensor plane
        ds.n    = trafo * Vector3f(0.0f, 0.0f, 1.0f);
        // either one or zero? TODO: check
    ...
    std::string to_string() const override {
        ...
        oss << "RadialWarpCamera[" << std::endl
            << "  x_fov = " << m_x_fov << "," << std::endl
            << "  k_warp = " << m_k_warp << "," << std::endl
...
private:
    ...
    Float ydot(Float y) const {
        return -m_k_warp * y * y;
    }

    Float solve_rk4(Float y0, Float tf = 1.0f, UInt32 N = 64U) const {
        Float y = y0;
        Float dt = tf / (N + 1);
        UInt32 itr = 0U;
        dr::Loop<dr::mask_t<UInt32>> loop("ode_int", itr, y);

        Float k1, k2, k3, k4;
        while(loop(dr::neq(itr, N + 1))) {
            k1 = ydot(y) * dt;
            k2 = ydot(y + 0.5f * k1) * dt;
            k3 = ydot(y + 0.5f * k2) * dt;
            k4 = ydot(y + k3) * dt;
            y += (k1 + k4) / 6.0f + (k2 + k3) / 3.0f;
            itr += 1;
        }
        return y;
    }

    Vector3f warp_direction(Vector3f d) const {
        Float d_rad = dr::sqrt(dr::sqr(d.x()) + dr::sqr(d.y()));
        Float rs_init = Float(d_rad);
        Float rs_out = solve_rk4(d_rad);
        Float scale = rs_out * dr::rcp(rs_init);
        Vector3f d_out = Vector3f(d.x() * scale, d.y() * scale, d.z());
        
        return dr::normalize(d_out);
    }
};

MI_IMPLEMENT_CLASS_VARIANT(RadialWarpCamera, ProjectiveCamera)
MI_EXPORT_PLUGIN(RadialWarpCamera, "Radial Camera");
